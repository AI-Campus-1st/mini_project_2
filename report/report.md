...
서울에서 카페 출점을 검토할 때 인구가 많은 지역이라는 이유만으로 입지를 결정하기는 어렵다. 지역에 머무는 인구가 많더라도 기존 카페가 밀집해 있을 수 있고, 거주인구가 많아도 낮 시간대의 활동 규모는 다를 수 있기 때문이다. 본 프로젝트는 생활인구, 카페 수, 거주인구를 결합하여 서울 25개 자치구 중 현장 조사를 우선 진행할 후보를 선정하는 대시보드를 구축했다.
...
분석 기준은 2026년 6월이며, 생활인구 관측 기간은 2026년 1월 1일부터 6월 30일까지이다. 수집한 시간대는 08~23시이고, 실제 지표 계산에는 프로젝트에서 영업시간대로 설정한 **08~22시의 15개 시간대**를 사용한다. 이는 모든 카페의 실제 영업시간을 조사한 결과가 아니라 지역 간 비교를 위한 공통 분석 범위이다.
...
출처와 기준일은 저장된 파일명 및 수집 코드에 근거한다. 생활인구는 거주인구와 함께 지역의 활동 규모를 살펴보기 위한 지표로 사용한다. 같은 사람이 여러 시간대에 포함될 수 있으므로 시간대별 값을 합쳐 하루의 고유 방문자 수나 카페 이용객 수로 해석하지 않는다.
...
수집기는 HTTPX의 비동기 클라이언트를 공유하고, 하루 단위의 요청을 `asyncio.gather`로 처리한다. 세마포어로 동시 요청을 5개로 제한하며 요청 타임아웃은 30초로 설정했다. 실패한 요청은 로그에 기록하고 빈 결과를 반환한다. 현재 코드에는 자동 재시도나 실패 구간부터 재개하는 기능이 없다.
...
```text
P(x) = (지표 x의 오름차순 최소 순위 − 1) / (자치구 수 − 1)
...
동점에는 최소 순위를 부여한다. 카페 수 대비 생활인구에 50%, 최근 변화에 30%, 활동 배율에 20%의 비중을 두었다. 이 가중치는 프로젝트의 비교 기준이며 매출 자료로 추정한 계수가 아니다. 점수는 서울 자치구 사이의 상대적 비교값으로, 사업 성공 확률을 의미하지 않는다.
...
조건 판단에 사용한 자치구별 카페당 생활인구의 산술평균은 **537.05명/개**이다. 이는 서울 전체 생활인구를 전체 카페 수로 나눈 값과 구분해야 한다.
...
마트만 초기화하고 새로 적재하려면 다음 명령을 사용한다.

```powershell
# .env의 DB_* 연결로 마트 재생성
python reset_marts.py

# .env의 TARGET_DB_* 연결로 대상 DB의 마트 재생성
python reset_marts.py --db TARGET_DB
```

`reset_marts.py`는 선택한 DB의 원본·정제 테이블을 읽어 마트 4개를 계산한다. 집계 결과가 비어 있는지, 결측값·무한대·기본키 중복이 있는지 검사한 뒤 해당 마트만 삭제·재생성한다. 삽입은 하나의 트랜잭션으로 수행하고 테이블별 적재 행 수를 확인한다. 원본·정제 테이블은 변경하지 않는다. MySQL의 테이블 삭제·생성은 암묵적으로 커밋되므로 이후 INSERT 실패 시 이전 마트가 복원되는 구조는 아니다.

`transfer_data.py`는 `DB_*`에서 `TARGET_DB_*`로 프로젝트 테이블의 구조와 데이터를 이전하는 별도 도구이다. 대상에 같은 이름의 테이블이 있으면 중단하므로, 이미 존재하는 마트 갱신에는 `reset_marts.py`를 사용한다.
...
본 보고서의 수치는 로컬 CSV 집계 결과이며 배포 DB에서 직접 조회한 수치가 아니다. 운영 DB 재적재와 배포 화면의 수치 일치 여부는 이번 보고서 작성 과정에서 검증하지 않았다.
...
운영 측면에서는 실패 요청 재시도, 수집 결과의 실행별 보관, 완전한 기간·시간대 조합 검증을 보완할 수 있다. 점수 기준월을 매개변수로 만들고 마트에 분석 기준월을 저장하면 기간별 결과를 추적할 수 있다.

현재 홈 화면은 활동 배율 조건을 “평균보다 높음”으로 설명하지만 실제 계산 기준은 “1배 초과”이다. 우선순위 점수 캡션에도 상대 순위 변환 과정이 생략되어 있다. 보고서의 산식은 집계 코드를 기준으로 작성했으며, 화면 설명도 이에 맞춰 통일할 필요가 있다. 후보명은 고정 문구로 작성되어 있으므로 데이터 갱신에 맞춘 자동 생성과 조건 충족 지역이 없을 때의 처리도 개선 대상이다.
...
| 마트 초기화와 DB 이전 | `reset_marts.py`, `transfer_data.py` |
...
AI는 마트 초기화 코드 작성, DB 연결·캐시 동작 검토, 파일 기반 보고서 정리와 수치 대조에 활용했다. 분석 결과는 로컬 데이터와 현재 집계 함수를 기준으로 확인했으며, 운영 환경에서 실행하지 않은 작업은 완료된 성과로 포함하지 않았다.
